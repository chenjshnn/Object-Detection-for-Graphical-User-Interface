...
def train(training_dbs, validation_db, num_classes, start_iter=0):
    ...
    data_file   = "sample.coco"#.format(training_dbs[0].data)
    ...
    nnet = NetworkFactory(training_dbs[0], num_classes)
    ...
    if pretrained_model is not None:
        ...
        #TODO need  to modify the num of class
    ...
    writer = SummaryWriter(system_configs.log_dir)
    nnet.cuda()
    nnet.train_mode()
    with stdout_to_tqdm() as save_stdout:
        for iteration in tqdm(range(start_iter + 1, max_iteration + 1), file=save_stdout, ncols=80):
            ...
            if display and iteration % display == 0:
                ...
                writer.add_scalar("train_loss/training loss", training_loss.item(), iteration)
                writer.add_scalar("train_loss/focal loss", focal_loss.item(), iteration)
                writer.add_scalar("train_loss/pull loss", pull_loss.item(), iteration)
                writer.add_scalar("train_loss/push loss", push_loss.item(), iteration)
                writer.add_scalar("train_loss/regr loss", regr_loss.item(), iteration)
            ...
            if val_iter and validation_db.db_inds.size and iteration % val_iter == 0:
                ...
                writer.add_scalar("val_loss=", validation_loss.item(), iteration)
                nnet.train_mode()
...
if __name__ == "__main__":
    ...
    train(training_dbs, validation_db, configs["db"]["categories"], args.start_iter)
