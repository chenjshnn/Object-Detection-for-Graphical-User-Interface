...
class RICO_DEMO(DETECTION):
    def __init__(self, db_config, split, test_folder):
        super(RICO_DEMO, self).__init__(db_config)
        data_dir   = system_configs.data_dir
        result_dir = system_configs.result_dir
        cache_dir  = system_configs.cache_dir

        print(data_dir, result_dir, cache_dir)
        print(system_configs.categories)
        self._coco_dir = data_dir #os.path.join(data_dir, "coco")
        self._split = split

        self.dataset = system_configs.dataset
        self._label_file = os.path.join(test_folder, "instances_tmp.json")
        ...
        self._cat_ids = list(range(system_configs.categories))
        print("cate ids(except background):", self._cat_ids)
        # [
        ...
        self._cache_file = os.path.join(cache_dir, "{}_{}.pkl".format(self.dataset, self._split))
        self._load_data()
    ...
    def convert_to_coco(self, all_bboxes):
        ...
        for image_id in all_bboxes:
            ...
            for cls_ind in all_bboxes[image_id]:
                print(all_bboxes[image_id].keys())
                category_id = cls_ind #self._classes[cls_ind] 
