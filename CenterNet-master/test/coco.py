...
def kp_detection(db, nnet, result_dir, debug=False, decode_func=kp_decode):
    ...
    nms_threshold = 0.2# db.configs["nms_threshold"]
    ...
    for ind in tqdm(range(0, num_images), ncols=80, desc="locating kps"):#
        ...
        for j in range(categories):
            keep_inds = (classes == j)
            top_bboxes[image_id][j] = detections[keep_inds][:, 0:7].astype(np.float32)
            if merge_bbox:
                soft_nms_merge(top_bboxes[image_id][j], Nt=nms_threshold, method=nms_algorithm, weight_exp=weight_exp)
            else:
                soft_nms(top_bboxes[image_id][j], Nt=nms_threshold, method=nms_algorithm)
            top_bboxes[image_id][j] = top_bboxes[image_id][j][:, 0:5]

        scores = np.hstack([
            top_bboxes[image_id][j][:, -1] 
            for j in range(categories)#range(1, categories + 1)
        ])
    ...
    cls_ids = list(range(categories))
    image_ids = [db.image_ids(ind) for ind in db_inds]
    db.evaluate(result_json, cls_ids, image_ids)
    return 0

def testing(db, nnet, result_dir, debug=True):
