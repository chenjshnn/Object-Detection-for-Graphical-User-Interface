...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="rico", help="dataset")
    
    opt = parser.parse_args()

    print(opt)

    log_dir = "results/run/{}/logs".format(opt.dataset)
    ...
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    run_path = "results/run/{}".format(opt.dataset)
    ...
    for epoch in range(opt.start_epoch, opt.epochs):
        ...
        if epoch % opt.checkpoint_interval == 0:
            torch.save(model.state_dict(), "results/run/{}/yolov3_ckpt_{}.pth".format(opt.dataset, epoch))
